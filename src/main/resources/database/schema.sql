...
CREATE TABLE project (
    ...
    status VARCHAR(50) NOT NULL,             -- Enum FundingStatus
    expected_delivery_date VARCHAR(255),     -- 배송 예상일
    deleted_at DATETIME,                      -- 삭제 일자
    creator_id BIGINT,                        -- Creator FK
    created_at        DATETIME(6),
    updated_at        DATETIME(6),
...
CREATE TABLE sponsorship (
    id BIGINT AUTO_INCREMENT PRIMARY KEY,
    amount BIGINT,                        -- 후원 금액
    sponsored_at DATETIME,                -- 후원 시점
    quantity INT,                         -- 선택한 리워드 갯수
    cancelled BOOLEAN NOT NULL DEFAULT FALSE,  -- 취소 여부
    user_id BINARY(16) NOT NULL,          -- 후원자 (User FK)
